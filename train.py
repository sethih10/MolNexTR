...
from MolNexTR.utils import seed_torch, save_args, init_summary_writer, LossMeter, AverageMeter, asMinutes, timeSince, print_rank_0, format_df
...
def get_args():
    ...
    paraser.add_argument('--no_pretrained', action='store_true')
...
def valid_fn(valid_loader, encoder, decoder, tokenizer, device, args):
    ...
    gathered_preds = [None for i in range(dis.get_world_size())]
...
def train_loop(args, train_df, valid_df, aux_df, tokenizer, save_path):
    ...
    encoder_optimizer, encoder_scheduler, decoder_optimizer, decoder_scheduler = get_optimizer_and_scheduler(args, encoder, decoder, load_path=args.load_path)
...
def inference(args, data_df, tokenizer, encoder=None, decoder=None, save_path=None, split='test'):
    ...
    if 'image_id' in data_df.columns:
    ...
    # Construct a graph from predicted atoms and bonds (including verifying chirality)
...
def main():
    ...
    if args.do_train:
        train_loop(args, valid_df, tokenizer, save_path=args.save_path)
